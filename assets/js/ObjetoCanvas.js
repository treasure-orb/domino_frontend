...
ObjetoCanvas.prototype.EnlazarEventos = function() {
    ...
    // Need to save a variable with each event reconverted with bind, to be able to do the removeEventListener later
    if (this.OpcionesCanvas.Entorno === "Normal") { // Canvas que puede recibir eventos  //Canvas that can receive events  Entro = Environment
        if (ObjetoNavegador.EsMovil() === true) {// ObjetoNavegador = ObjectBrowser, EsMovil = isMobile
            ...
            this.Cabecera.addEventListener('touchstart', this.hEventoTouchStart);//Cabecera = Headboard
            ...
            this.hEventoMousePresionado = this.EventoMousePresionado.bind(this); //Presiondo = Pressed
            this.hEventoMouseSoltado    = this.EventoMouseSoltado.bind(this); //Soltada = unleashed
        ...
        this.hEventoTeclaPresionada = this.EventoTeclaPresionada.bind(this); //Tecla = Key
    ...
    this.hEventoRedimensionar   = this.EventoRedimensionar.bind(this); //Redimensionar = Resize
    this.hEventoScroll          = this.EventoScroll.bind(this);
    this.hEventoFocoPerdido     = this.EventoFocoPerdido.bind(this); //Foco = Focus, Perdido = Lost,
    this.hEventoFocoRecibido    = this.EventoFocoRecibido.bind(this);//Recibido = Received
    if (ObjetoNavegador.EsMovil() === false) {
    ...
    window.addEventListener('resize', this.hEventoRedimensionar);//Redimensionar = Resize
    window.addEventListener('scroll', this.hEventoScroll);
    window.addEventListener('blur', this.hEventoFocoPerdido); 
    window.addEventListener('focus', this.hEventoFocoRecibido);       //Recibido = Received 
};
...
// Function that obtains the size of the canvas once it is resized.
...
//    console.log("esmovil" + ObjetoNavegador.EsMovil());
    ...
    if (this.OpcionesCanvas.ForzarLandscape === true && ObjetoNavegador.EsMovil() === true && window.innerWidth < window.innerHeight) {
        ...
        // I invert the new width and height (if they are not fixed) to force landscape mode
        //Ancho = width, Alto = height, Opciones=options
...
/*        this.Cabecera.style.width = this.Ancho + "px";
    ...
    // landscape
        ...
        if (this.OpcionesCanvas['Entorno'] === 'Normal') { //Entorno = Environment
        ...
        // Calculate the new width and the new height (if they are not fixed)
    ...
    // Assign the height and width to the attributes of the canvas
    this.Canvas.setAttribute("width", this.Ancho);
    this.Canvas.setAttribute("height", this.Alto);
    if (this.OpcionesCanvas.Tipo.toLowerCase() === "three") { // redimensionar el THREE.JS, redimensionar = resize,
        this.Context.setSize(this.Ancho, this.Alto);
        if (this.Camara !== null && typeof(this.Camara) !== 'undefined') { // Si hay una camara creada //If there is a camera created
            this.Camara.aspect = this.Ancho / this.Alto;
            this.Camara.updateProjectionMatrix();            
        }
    }
    // 
    if (typeof(this.Redimensionar) !== "undefined") {//redimensionar = resize,
...
// Object to detect current browser
...
//                console.log("ObjetoCanvas.EsMovil : false");
    ...
    // Returns true if it is a firefox browser
